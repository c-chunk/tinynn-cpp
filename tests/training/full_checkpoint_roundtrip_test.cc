#include <cstdint>
#include <cstring>
#include <iostream>
#include <string>
#include <vector>

#include <tinynn/nn/sequential.h>
#include <tinynn/nn/layers/affine.h>
#include <tinynn/nn/layers/relu.h>
#include <tinynn/nn/layers/batch_norm1d.h>
#include <tinynn/nn/losses/softmax_with_loss.h>
#include <tinynn/optim/adam.h>
#include <tinynn/training/trainer.h>
#include <tinynn/training/callback.h>
#include <tinynn/training/callbacks/checkpoint_state_io.h>

// ---- tiny deterministic loader ----
namespace {

template <class T>
struct Batch {
  tinynn::Tensor<T> x;
  std::vector<tinynn::SizeType> y;
};

// yields fixed set of batches every epoch (no shuffle)
template <class T>
class FixedLoader {
 public:
  explicit FixedLoader(std::vector<Batch<T>> batches) : batches_(std::move(batches)) {}

  void set_epoch(int) {}     // optional hook used by Trainer
  void prepare_epoch() {}    // optional hook used by Trainer

  auto begin() const { return batches_.begin(); }
  auto end() const { return batches_.end(); }

 private:
  std::vector<Batch<T>> batches_;
};

// ---- callback with checkpointable state ----
template <class T>
class StepCounterCallback final : public tinynn::TrainerCallback<T> {
 public:
  const char* checkpoint_tag() const noexcept override { return "StepCounter"; }

  void on_train_step_end(tinynn::Trainer<T>&, int, tinynn::SizeType, const tinynn::StepResult<T>&) override {
    ++count_;
  }

  void save_state(tinynn::CheckpointWriter& w) const override {
    w.write_u64(static_cast<uint64_t>(count_));
  }
  void load_state(tinynn::CheckpointReader& r) override {
    count_ = static_cast<std::uint64_t>(r.read_u64());
  }

  std::uint64_t count() const noexcept { return count_; }

 private:
  std::uint64_t count_ = 0;
};

template <class T>
std::vector<T> snapshot_params(tinynn::Sequential<T>& model) {
  std::vector<tinynn::ParameterView<T>> pvs;
  model.collect_parameter_views(pvs);

  std::vector<T> flat;
  for (auto& pv : pvs) {
    auto p = pv.param;
    for (tinynn::SizeType i = 0; i < p.size(); ++i) {
      flat.push_back(p[i]);
    }
  }
  return flat;
}

template <class T>
void assert_params_equal(const std::vector<T>& a, const std::vector<T>& b, const char* what) {
  if (a.size() != b.size()) {
    throw std::runtime_error(std::string("[NG] ") + what + ": param size mismatch");
  }
  for (size_t i = 0; i < a.size(); ++i) {
    if (std::memcmp(&a[i], &b[i], sizeof(T)) != 0) {
      // exact compare (bitwise). If you ever want tolerance, switch here.
      throw std::runtime_error(std::string("[NG] ") + what + ": mismatch at i=" + std::to_string(i));
    }
  }
}

}  // namespace

int main() {
  using T = double;
  try {
    // deterministic tiny dataset: XOR-ish (linearly non-separable, BN+ReLU helps)
    // We'll create 4 samples and pack them into 1 batch of B=4
    const tinynn::SizeType B = 4;
    const tinynn::SizeType in_dim = 2;
    const tinynn::SizeType hidden = 16;
    const tinynn::SizeType out_dim = 2;

    Batch<T> b0{ tinynn::Tensor<T>({B, in_dim}), std::vector<tinynn::SizeType>(B) };
    // x
    b0.x(0,0)=0; b0.x(0,1)=0;
    b0.x(1,0)=0; b0.x(1,1)=1;
    b0.x(2,0)=1; b0.x(2,1)=0;
    b0.x(3,0)=1; b0.x(3,1)=1;
    // labels (XOR)
    b0.y[0]=0; b0.y[1]=1; b0.y[2]=1; b0.y[3]=0;

    FixedLoader<T> loader({b0});  // same for train/eval

    // -------- Trainer A (reference) --------
    tinynn::Sequential<T> modelA;
    modelA.add(std::make_unique<tinynn::Affine<T>>(in_dim, hidden));
    modelA.add(std::make_unique<tinynn::BatchNorm1d<T>>(hidden));
    modelA.add(std::make_unique<tinynn::ReLU<T>>());
    modelA.add(std::make_unique<tinynn::Affine<T>>(hidden, out_dim));

    // IMPORTANT: explicit init (avoid all-zero dead ReLU)
    modelA.for_each_layer([&](tinynn::Layer<T>& lyr) {
      if (auto* a = dynamic_cast<tinynn::Affine<T>*>(&lyr)) {
        a->init_he_normal(123);
      }
    });

    tinynn::SoftmaxWithLoss<T> lossA;
    tinynn::Adam<T> optA(/*lr=*/static_cast<T>(1e-2), /*beta1=*/static_cast<T>(0.9),
                         /*beta2=*/static_cast<T>(0.999), /*eps=*/static_cast<T>(1e-8),
                         /*weight_decay=*/static_cast<T>(1e-4));

    tinynn::Trainer<T> trainerA(modelA, lossA, optA);
    StepCounterCallback<T> cbA;
    trainerA.add_callback(&cbA);

    // run some steps (2 epochs x 1 batch = 2 steps)
    for (int e = 0; e < 2; ++e) {
      (void)trainerA.train_epoch(loader, e);
      (void)trainerA.eval_epoch(loader, e);
    }

    const std::string path = "full_ckpt_roundtrip.bin";
    const int next_epoch = 2;
    tinynn::save_full_checkpoint<T>(path, trainerA, next_epoch);

    // continue training further on A (e.g., 3 more epochs)
    for (int e = 2; e < 5; ++e) {
      (void)trainerA.train_epoch(loader, e);
      (void)trainerA.eval_epoch(loader, e);
    }

    const auto ref_params = snapshot_params(modelA);
    const auto ref_global_step = trainerA.global_step();
    const auto ref_cb_count = cbA.count();

    // -------- Trainer B (restore) --------
    tinynn::Sequential<T> modelB;
    modelB.add(std::make_unique<tinynn::Affine<T>>(in_dim, hidden));
    modelB.add(std::make_unique<tinynn::BatchNorm1d<T>>(hidden));
    modelB.add(std::make_unique<tinynn::ReLU<T>>());
    modelB.add(std::make_unique<tinynn::Affine<T>>(hidden, out_dim));

    // init is irrelevant because we load parameters, but keep deterministic anyway
    modelB.for_each_layer([&](tinynn::Layer<T>& lyr) {
      if (auto* a = dynamic_cast<tinynn::Affine<T>*>(&lyr)) {
        a->init_he_normal(999);  // intentionally different
      }
    });

    tinynn::SoftmaxWithLoss<T> lossB;
    tinynn::Adam<T> optB(/*lr=*/static_cast<T>(1e-1), /*beta1=*/static_cast<T>(0.5),
                         /*beta2=*/static_cast<T>(0.5), /*eps=*/static_cast<T>(1e-3),
                         /*weight_decay=*/static_cast<T>(0));  // intentionally different

    tinynn::Trainer<T> trainerB(modelB, lossB, optB);
    StepCounterCallback<T> cbB;
    trainerB.add_callback(&cbB);

    int restored_next_epoch = -1;
    tinynn::load_full_checkpoint<T>(path, trainerB, &restored_next_epoch);

    if (restored_next_epoch != next_epoch) {
      throw std::runtime_error("[NG] next_epoch mismatch");
    }

    // resume training from restored_next_epoch up to the same final epoch as A
    for (int e = restored_next_epoch; e < 5; ++e) {
      (void)trainerB.train_epoch(loader, e);
      (void)trainerB.eval_epoch(loader, e);
    }

    const auto got_params = snapshot_params(modelB);
    const auto got_global_step = trainerB.global_step();
    const auto got_cb_count = cbB.count();

    // assertions (exact match)
    assert_params_equal(ref_params, got_params, "full checkpoint params");
    if (ref_global_step != got_global_step) {
      throw std::runtime_error("[NG] global_step mismatch");
    }
    if (ref_cb_count != got_cb_count) {
      throw std::runtime_error("[NG] callback counter mismatch");
    }

    std::cout << "[OK] full checkpoint roundtrip matched (params/global_step/callback)\n";
    std::cout << "full_checkpoint_roundtrip_test passed.\n";
    return 0;

  } catch (const std::exception& e) {
    std::cerr << e.what() << "\n";
    return 1;
  }
}