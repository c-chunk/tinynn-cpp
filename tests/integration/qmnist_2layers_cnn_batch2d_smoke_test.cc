...
int main() {
  ...
  model.add(std::make_unique<tinynn::BatchNorm2d<T>>(/*num_channels=*/8, /*eps=*/1e-5f, /*momentum=*/0.1f));
  ...
  model.add(std::make_unique<tinynn::BatchNorm2d<T>>(/*num_channels=*/16, /*eps=*/1e-5f, /*momentum=*/0.1f));
  model.add(std::make_unique<tinynn::ReLU<T>>());
  ...
  std::cout << "QMNIST 2-layer CNN (+BN2d) smoke test OK.\n";
  return 0;
}
