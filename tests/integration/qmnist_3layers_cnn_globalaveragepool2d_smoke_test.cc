...
#include <tinynn/nn/layers/global_average_pool2d.h>
...
int main() {
  ...
  std::cout << "train size=" << train_ds.size() << " rows=" << train_ds.rows()
            << " cols=" << train_ds.cols() << "\n";
  std::cout << "test  size=" << test_ds.size() << " rows=" << test_ds.rows()
            << " cols=" << test_ds.cols() << "\n";
  ...
  // ---- Model: 3-layer CNN (+BN2d + GlobalAveragePool2d) ----
  ...
  model.add(std::make_unique<tinynn::BatchNorm2d<T>>(
      /*num_channels=*/8, /*eps=*/1e-5f, /*momentum=*/0.1f));
  model.add(std::make_unique<tinynn::ReLU<T>>());
  model.add(std::make_unique<tinynn::MaxPool2d<T>>(
      tinynn::MaxPool2dOptions{}));  // [28,28] -> [14,14]
  ...
  model.add(std::make_unique<tinynn::BatchNorm2d<T>>(
      /*num_channels=*/32, /*eps=*/1e-5f, /*momentum=*/0.1f));
  model.add(std::make_unique<tinynn::ReLU<T>>());
  ...
  model.add(std::make_unique<tinynn::BatchNorm2d<T>>(
      /*num_channels=*/64, /*eps=*/1e-5f, /*momentum=*/0.1f));
  model.add(std::make_unique<tinynn::ReLU<T>>());

  // [B,64,14,14] -> [B,64,1,1]
  model.add(std::make_unique<tinynn::GlobalAveragePool2d<T>>());

  // [B,64,1,1] -> [B,64]
  model.add(std::make_unique<tinynn::Flatten<T>>());

  // classifier
  model.add(std::make_unique<tinynn::Affine<T>>(64, 10, true));

  // Initialize all trainable layers using each layer's default policy.
  model.reset_parameters();

  // ---- Trainer ----
  tinynn::SoftmaxWithLoss<T> loss_fn;

  tinynn::SGD<T> sgd(/*lr=*/0.03f, /*weight_decay=*/0.0f);
  ...
  std::cout
      << "QMNIST 3-layer CNN (+BN2d + GlobalAveragePool2d) smoke test OK.\n";
  return 0;
}
