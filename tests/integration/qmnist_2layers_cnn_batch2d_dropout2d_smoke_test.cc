...
int main() {
  ...
  // ---- Model: 2-layer CNN (+ BatchNorm2d + Dropout2d) ----
  ...
  model.add(std::make_unique<tinynn::MaxPool2d<T>>(
      tinynn::MaxPool2dOptions{}));  // 2x2 stride2

  // Block 2: Conv -> BN -> ReLU -> Pool -> Dropout2d
  model.add(std::make_unique<tinynn::Conv2d<T>>(tinynn::Conv2dOptions{
      .in_channels = 8,
      .out_channels = 16,
      .kernel_h = 3,
  ...
  model.add(std::make_unique<tinynn::BatchNorm2d<T>>(
      /*num_channels=*/16, /*eps=*/1e-5f, /*momentum=*/0.1f));
  model.add(std::make_unique<tinynn::ReLU<T>>());
  model.add(std::make_unique<tinynn::MaxPool2d<T>>(
      tinynn::MaxPool2dOptions{}));
  model.add(std::make_unique<tinynn::Dropout2d<T>>(
      /*drop_prob=*/0.20f, /*seed=*/12345ULL));
  ...
  std::cout << "QMNIST 2-layer CNN (+BN2d + Dropout2d) smoke test OK.\n";
  return 0;
}
